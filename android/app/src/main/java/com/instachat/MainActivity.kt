...
class MainActivity : ReactActivity() {
  ...
  private fun extractUrlFromText(text: String): String {
    ...
    return if (matcher.find()) {
      ...
      Log.d("ShareIntent", "Extracted URL from shared text")
      url
    } else {
      // No URL found, return original text (might be a plain URL)
      Log.d("ShareIntent", "No URL regex match in shared text")
      text.trim()
    }
  }
  ...
  private fun handleShareIntentSafely(intent: Intent) {
    if (intent.action == Intent.ACTION_SEND && intent.type?.startsWith("text/") == true) {
      ...
      Log.d("ShareIntent", "Received ACTION_SEND text share type=${intent.type}")
